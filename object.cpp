...
Object::Object(Shape* _shape, const int _objectId,
               const glm::vec3 _diffuseColor, const glm::vec3 _specularColor, const float _shininess, Texture* texture, Texture* normal)
    : diffuseColor(_diffuseColor), specularColor(_specularColor), shininess(_shininess),
	shape(_shape), objectId(_objectId), drawMe(true), texture(texture), normal(normal)
...
void Object::Draw(ShaderProgram* program, glm::mat4& objectTr)
{
    ...
    glUniform1i(loc, texture ? 1 : 0);

	loc = glGetUniformLocation(program->programId, "hasNormalMap");
    glUniform1i(loc, normal ? 1 : 0);

    // Draw this object
    CHECKERROR;
    if (shape)
        if (drawMe) {
            if (texture)    texture->BindTexture(0, program->programId, "tex"); 
			if (normal)	    normal->BindTexture(2, program->programId, "normalMap");
			shape->DrawVAO();
            if (texture)    texture->UnbindTexture(0);
            if (normal)     normal->UnbindTexture(2);
        }
