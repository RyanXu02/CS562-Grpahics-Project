...
static inline GLint castGLint(GLenum e) {
    return (int)e;
}//static_cast<GLint>(e); }
...
void FBO::CreateGbufferFBO(const int w, const int h)
{
    width = w;
    height = h;
    ...
    // 0: World Position vec3 (RGBA32F)
    glBindTexture(GL_TEXTURE_2D, textureID[0]);
    glTexImage2D(GL_TEXTURE_2D, 0, castGLint(GL_RGBA32F),
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, castGLint(GL_NEAREST));
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, castGLint(GL_NEAREST));
    glFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT,
        GL_COLOR_ATTACHMENT0_EXT,
        GL_TEXTURE_2D,
        textureID[0], 0);

    // 1: World Normal vec3 (RGBA32F)
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, castGLint(GL_RGBA8),
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, castGLint(GL_RGBA16F),
        width, height, 0, GL_RGBA, GL_FLOAT, NULL);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAX_LEVEL, 0);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, castGLint(GL_CLAMP_TO_EDGE));
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, castGLint(GL_CLAMP_TO_EDGE));
...
void FBO::BindTexture(const int index, const int unit, const int programId, const std::string& name)
{
